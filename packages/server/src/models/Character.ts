...
export interface ICharacter extends Document {
  ...
  consonant?: '-' | 'k' | 'g' | 's' | 'z' | 't' | 'd' | 'n' | 'h' | 'b' | 'p' | 'm' | 'y' | 'r' | 'w' | null;
...
CharacterSchema.pre('save', function(this: ICharacter, next: Function) {
  if (this.vowel === null && this.consonant === null && this.translation === null && this.japanese_pronunciation === null) {
    return next(new Error('Vowel, consonant, translation and japanese pronunciation cannot all be null at the same time.'));
  }
  next();
});
